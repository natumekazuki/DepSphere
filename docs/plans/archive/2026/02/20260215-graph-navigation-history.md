...
## Task List
- [x] UIに戻る/進むボタンを追加する。
- [x] 選択・フィルタ・カメラ状態を履歴スナップショットとして保持する。
- [x] 戻る/進む適用時に状態復元（表示/選択/視点）を実装する。
- [x] 既存操作（クリック/検索/Fit/解除）と履歴記録の接続を行う。
- [x] テストを更新して回帰確認する。
...
## Notes / Logs
- 2026-02-15: 初版作成。
- 2026-02-15: 戻る/進むボタンと履歴スナップショット（選択・フィルタ・カメラ）を実装。
- 2026-02-15: `Alt+Left/Alt+Right` の履歴ショートカットを追加。
- 2026-02-15: `dotnet test tests/DepSphere.Analyzer.Tests/DepSphere.Analyzer.Tests.csproj -v minimal` 実行、66件成功。
