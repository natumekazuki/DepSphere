...
- ノード:
  ...
  - 外部依存は `External` 集約ノードへ接続し、`external` エッジで可視化する（判定不能な外部型はノード生成しない）
...
- エッジ:
  ...
  - 終点ノード手前に矢印ヘッドを配置し、`from -> to` の方向を表示
- 基本操作:
  ...
  - `自動拡散` ボタンで表示中ノード数/エッジ密度から距離倍率を自動調整し、必要なら既定上限を超えて拡散する
  - 左上の操作オーバーレイ内ヘッダーの `▽/△` トグルで、操作UI（検索/履歴/Fit/表示限定解除）を折りたたみ/展開できる
  - 右上のノード情報オーバーレイ内ヘッダーの `▽/△` トグルで、ノード情報表示を折りたたみ/展開できる（デフォルトは折りたたみ）
  - ノード選択、検索、フォーカス
...
- 解析入力導線:
  ...
  - 起動時はサンプルを自動描画せず、プレースホルダー状態で待機する。
...
- [ ] AC-WPF-02: 起動後はプレースホルダーが表示され、初期状態ではサンプル解析が自動実行されない。
...
- [ ] AC-WPF-07: 操作パネルに `進捗更新間隔` 入力欄が表示されず、解析は既定値（25）で実行される。
...
- [ ] AC-WPF-10: `再解析` 押下で直近の解析パスを再実行でき、未指定時はガイダンス表示となる。
- [ ] AC-WPF-11: ヘッダーの `操作パネルを隠す/表示` で左ペインが切り替わり、再表示時に幅が復元される。
- [ ] AC-WPF-12: 左-中央、中央-右の区切りをドラッグするとペイン幅が調整できる。
- [ ] AC-WPF-13: コードシンボルリンクで複数ノードへ遷移した後、`コード戻る` / `コード進む` でコード表示とグラフフォーカスが履歴移動できる。

### 15.3 判定
- 上記13項目がすべて満たされることを受け入れ条件とする。
